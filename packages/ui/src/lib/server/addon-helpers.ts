...
async function performAddonToggleMutation(
	...
	if (!nextEnabled && wasEnabled) {
		...
		if (serviceNames.length > 0) {
			...
			if (dockerCheck.ok) {
				try {
					await activateComposeCommand(state, ['stop', ...serviceNames], { lock });
...
export function performVoiceEngage(
	...
	return withAdminUpdateLock(state, requestId, async (lock, deferReleaseUntil) => {
		const result = await engageVoiceAddon({ state, wantsVoiceAddon: true, requestedProfile, lock });
			...
			case 'error':
				...
				return jsonResponse(
					502,
					{
						...
						enabled:
							result.wasAlreadyEnabled || result.steps.some((s) => s.step === 'enable' && s.ok),
...
export function handleAddonToggleRequest(
	...
	if (name === VOICE_ADDON && requestedEnabled !== false) {
		...
		if (requestedProfile && !isEnabled) {
			...
			const known = getAddonProfiles(state.homeDir, VOICE_ADDON).some(
				(p) => p.id === requestedProfile
			);
			if (!known) {
				return Promise.resolve(
					errorResponse(
						400,
						'invalid_profile',
						`Unknown voice profile "${requestedProfile}"`,
						{},
						requestId
					)
				);
			...
			return withAdminUpdateLock(state, requestId, () => {
				...
				return jsonResponse(
					200,
					{ ok: true, addon: VOICE_ADDON, enabled: false, changed: false },
					requestId
				);
			});
		}
