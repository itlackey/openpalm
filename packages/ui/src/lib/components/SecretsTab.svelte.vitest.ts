...
const emptyVault = { keys: [], vaultRef: 'vault:user', available: true };
const vaultWithKeys = { keys: ['GROQ_API_KEY', 'OPENAI_API_KEY'], vaultRef: 'vault:user', available: true };
const unavailableVault = { keys: [], vaultRef: 'vault:user', available: false };
...
describe('SecretsTab — vault available, no keys', () => {
  ...
  test('shows empty state when vault has no keys', async () => {
    render(SecretsTab, { props: { tokenStored: true } });
    await expect.element(page.getByText(/no keys in the user vault/i)).toBeVisible({ timeout: 3000 });
  });
});
...
describe('SecretsTab — vault unavailable', () => {
  test('shows unavailability banner when available=false', async () => {
    ...
    render(SecretsTab, { props: { tokenStored: true } });
    await expect.element(page.getByText(/akm vault is unavailable/i)).toBeVisible({ timeout: 3000 });
  });

  test('"Add / Update Key" button is disabled when vault unavailable', async () => {
    vi.mocked(fetchUserVault).mockResolvedValue(unavailableVault);
    render(SecretsTab, { props: { tokenStored: true } });
    await expect.element(page.getByRole('button', { name: /add \/ update key/i })).toBeDisabled({ timeout: 3000 });
  });
});
...
describe('SecretsTab — key list', () => {
  test('renders each key in the vault', async () => {
    ...
    render(SecretsTab, { props: { tokenStored: true } });
    await expect.element(page.getByText('GROQ_API_KEY')).toBeVisible({ timeout: 3000 });
    await expect.element(page.getByText('OPENAI_API_KEY')).toBeVisible({ timeout: 3000 });
  });
});
...
describe('SecretsTab — write form', () => {
  ...
  test('clicking "Add / Update Key" reveals the write form', async () => {
    render(SecretsTab, { props: { tokenStored: true } });
    await expect.element(page.getByRole('button', { name: /add \/ update key/i })).toBeVisible({ timeout: 3000 });
