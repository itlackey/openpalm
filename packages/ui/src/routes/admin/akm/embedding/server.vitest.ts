...
afterEach(() => {
	...
	describe('POST /admin/akm/embedding/detect', () => {
		test('401 without auth', async () => {
			expect((await detectPOST(makeEvent('/admin/akm/embedding/detect', {}, '') as Parameters<typeof detectPOST>[0])).status).toBe(401);
		});
	...
	test('returns detected settings', async () => {
		vi.mocked(detectEmbeddingSettings).mockResolvedValue({
			...
			const res = await detectPOST(makeEvent('/admin/akm/embedding/detect', {}) as Parameters<typeof detectPOST>[0]);
	...
	describe('POST /admin/akm/embedding/test', () => {
		test('400 for missing endpoint/model', async () => {
			expect((await testPOST(makeEvent('/admin/akm/embedding/test', { endpoint: '', model: '' }) as Parameters<typeof testPOST>[0])).status).toBe(400);
		});
	...
	test('returns the probed dimension', async () => {
		vi.mocked(testEmbeddingSettings).mockResolvedValue({
			...
			const res = await testPOST(makeEvent('/admin/akm/embedding/test', {
				endpoint: 'http://localhost:11434/v1/embeddings',
				model: 'mxbai-embed-large:latest',
				provider: 'ollama',
