/**
 * Thin route tests for POST /admin/containers/up (3.4 — mutating-endpoint coverage).
 *
...
const composeStartMock = vi.fn<() => Promise<{ ok: boolean; stdout: string; stderr: string; code: number }>>();
...
vi.mock('@openpalm/lib', async () => {
  ...
  return {
    ...actual,
    composeStart: (...args: unknown[]) => composeStartMock(...(args as [])),
...
function makePostEvent(token = 'admin-token', body: unknown = { service: 'assistant' }): Parameters<typeof POST>[0] {
  return {
    request: new Request('http://localhost/admin/containers/up', {
      method: 'POST',
...
beforeEach(() => {
  ...
  composeStartMock.mockResolvedValue({ ok: true, stdout: '', stderr: '', code: 0 });
});
...
describe('POST /admin/containers/up', () => {
  ...
  test('returns 500 docker_error when compose start fails', async () => {
    composeStartMock.mockResolvedValue({ ok: false, stdout: '', stderr: 'no such service', code: 1 });
    const res = await POST(makePostEvent());
