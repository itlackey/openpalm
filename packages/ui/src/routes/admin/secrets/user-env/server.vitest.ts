/**
 * Tests for the /admin/secrets/user-env route.
 *
...
describe('admin user-env route', () => {
  test('GET returns 401 without admin token', async () => {
    const res = await GET(makeEvent('GET', '/admin/secrets/user-env', undefined, ''));
    expect(res.status).toBe(401);
  });

  test('GET lists user env keys without exposing values', async () => {
    await POST(makeEvent('POST', '/admin/secrets/user-env', { key: 'CUSTOM_KEY', value: 'v1' }));
    await POST(makeEvent('POST', '/admin/secrets/user-env', { key: 'OTHER_KEY', value: 'v2' }));

    const res = await GET(makeEvent('GET', '/admin/secrets/user-env'));
  ...
  test('DELETE removes a key from the user env entirely', async () => {
    ...
    const res = await DELETE(makeEvent('DELETE', '/admin/secrets/user-env?key=DROP_ME'));
    expect(res.status).toBe(200);
  ...
  test('DELETE followed by GET no longer lists the key', async () => {
    await POST(makeEvent('POST', '/admin/secrets/user-env', { key: 'KEEP_ME', value: 'ok' }));
    await POST(makeEvent('POST', '/admin/secrets/user-env', { key: 'DROP_ME', value: 'bye' }));
    await DELETE(makeEvent('DELETE', '/admin/secrets/user-env?key=DROP_ME'));

    const listRes = await GET(makeEvent('GET', '/admin/secrets/user-env'));
  ...
  test('written user env file is mode 0600', async () => {
    await POST(makeEvent('POST', '/admin/secrets/user-env', { key: 'TOKEN', value: 'x' }));
    const state = getState();
