...
// /api/host/voice's auto-enable + compose-up + healthcheck flow doesn't
...
function makeGetEvent(token = 'admin-token'): Parameters<typeof GET>[0] {
	return {
		request: new Request('http://localhost/api/host/voice', {
			headers: {
...
describe('PUT /api/host/voice — host fallback overlays', () => {
