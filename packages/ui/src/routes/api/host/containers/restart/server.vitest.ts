...
const composeRestartMock = vi.fn<() => Promise<void>>();
...
vi.mock('@openpalm/lib', async () => {
	...
	return {
		...actual,
		activateComposeCommand: (...args: unknown[]) => composeRestartMock(...(args as [])),
...
beforeEach(() => {
	...
	composeRestartMock.mockResolvedValue(undefined);
});
...
describe('POST /api/host/containers/restart', () => {
	...
	test('returns 500 docker_error when compose restart fails', async () => {
		composeRestartMock.mockRejectedValue(new Error('no such service'));
		const res = await POST(makePostEvent());
