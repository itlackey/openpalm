/**
 * Thin route tests for POST /api/host/containers/restart (3.4 — mutating-endpoint coverage).
 *
 * Previously untested. Mirrors containers/up: auth gate, invalid service
 * rejection, docker-unavailable soft-success, docker success, docker error,
 * and host-swap-blocked 409.
 */
import { afterEach, beforeEach, describe, expect, test, vi } from 'vitest';

const composeRestartMock = vi.fn<() => Promise<{ ok: boolean; stdout: string; stderr: string; code: number }>>();
...
vi.mock('@openpalm/lib', async () => {
  ...
  return {
    ...actual,
    composeRestart: (...args: unknown[]) => composeRestartMock(...(args as [])),
...
function makePostEvent(token = 'admin-token', body: unknown = { service: 'assistant' }): Parameters<typeof POST>[0] {
  return {
    request: new Request('http://localhost/api/host/containers/restart', {
      method: 'POST',
...
beforeEach(() => {
  ...
  composeRestartMock.mockResolvedValue({ ok: true, stdout: '', stderr: '', code: 0 });
});
...
describe('POST /api/host/containers/restart', () => {
  ...
  test('restarts the service and returns running status when docker succeeds', async () => {
    ...
    expect(composeRestartMock).toHaveBeenCalledOnce();
  });
  ...
  test('returns 500 docker_error when compose restart fails', async () => {
    composeRestartMock.mockResolvedValue({ ok: false, stdout: '', stderr: 'no such service', code: 1 });
    const res = await POST(makePostEvent());
