/**
 * Thin route tests for POST /api/host/containers/down (3.4 — mutating-endpoint coverage).
 *
...
const composeStopMock = vi.fn<() => Promise<void>>();
...
vi.mock('@openpalm/lib', async () => {
	...
	return {
		...actual,
		activateComposeCommand: (...args: unknown[]) => composeStopMock(...(args as [])),
...
function makePostEvent(
	...
	return {
		request: new Request('http://localhost/api/host/containers/down', {
			method: 'POST',
...
beforeEach(() => {
	...
	composeStopMock.mockResolvedValue(undefined);
});
...
describe('POST /api/host/containers/down', () => {
	...
	test('returns 500 docker_error when compose stop fails', async () => {
		composeStopMock.mockRejectedValue(new Error('no such service'));
		const res = await POST(makePostEvent());
