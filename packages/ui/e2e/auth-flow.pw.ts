 ...
 *   - a mutating /admin/containers/* endpoint enforces the same auth gate
...
test.describe('Auth flow — login / session / logout (mocked-lib)', () => {
  ...
  test('missing password body returns 400', async ({ request }) => {
    const res = await request.post('/admin/auth/login', { data: {} });
  ...
  test('login → protected read → logout → protected read is 401 again', async ({ request }) => {
    ...
    const listRes = await request.get('/admin/containers/list');
    expect(listRes.status()).not.toBe(401);
    expect(listRes.status()).toBeLessThan(500);

    // 2. Logout revokes the session.
    const logoutRes = await request.post('/admin/auth/logout');
    expect(logoutRes.status()).toBe(200);

    // 3. The same cookie jar's now-revoked session must be rejected.
    const afterLogout = await request.get('/admin/containers/list');
    expect(afterLogout.status()).toBe(401);
  });
});

test.describe('Auth flow — mutating containers/* endpoint enforces auth (mocked-lib)', () => {
  test('POST /admin/containers/up returns 401 with no auth', async ({ request }) => {
    const res = await request.post('/admin/containers/up', {
      data: { service: 'assistant' },
    });
    expect(res.status()).toBe(401);
  });

  test('POST /admin/containers/up returns 401 with a forged cookie', async ({ request }) => {
    const res = await request.post('/admin/containers/up', {
