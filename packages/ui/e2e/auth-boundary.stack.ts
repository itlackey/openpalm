...
test.describe('Auth boundary — write endpoints', () => {
  ...
  test('POST /admin/secrets/user-vault returns 401 without auth', async ({ request }) => {
    const res = await request.post(`${ADMIN_URL}/admin/secrets/user-vault`, {
  ...
  test('POST /admin/secrets/user-vault returns 401 with wrong cookie', async ({ request }) => {
    const res = await request.post(`${ADMIN_URL}/admin/secrets/user-vault`, {
  ...
  test('DELETE /admin/secrets/user-vault returns 401 without auth', async ({ request }) => {
    const res = await request.delete(`${ADMIN_URL}/admin/secrets/user-vault?key=E2E_TEST`, {
      headers: noAuth(),
    });
    expect(res.status()).toBe(401);
  });
