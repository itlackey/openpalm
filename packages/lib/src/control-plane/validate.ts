...
export async function validateProposedState(state: ControlPlaneState): Promise<{
  ...
  for (const key of REQUIRED_SECRET_KEYS) {
    ...
    if (!value || value.trim().length === 0) {
      errors.push(`ERROR: required secret ${key} is missing or empty in config/stack/secrets/${key.toLowerCase()}`);
    }
  }
  ...
  for (const mapping of getCoreSecretMappings(runtimeEnv)) {
    ...
    if (!inRuntime) {
      warnings.push(
        `WARN: ${mapping.envKey} (akm ${mapping.secretKey}) is not declared in config/stack/secrets/${mapping.envKey.toLowerCase()}`,
      );
    }
  }
