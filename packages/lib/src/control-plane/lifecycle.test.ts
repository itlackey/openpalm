...
import { makeTempDir, makeTestState, trackDir, registerCleanup } from "../../../admin/src/lib/server/test-helpers.js";
...
// These tests call reconcileCore which reads core.compose.yml via
// resolveOpenPalmHome() (OP_HOME). We must point OP_HOME at the temp dir,
// seed a minimal compose file, and skip Docker preflight.

function setupLifecycleTest(state: ReturnType<typeof makeTestState>): string | undefined {
  const origHome = process.env.OP_HOME;
...
function teardownLifecycleTest(origHome: string | undefined): void {
  if (origHome === undefined) delete process.env.OP_HOME;
  else process.env.OP_HOME = origHome;
...
describe("applyInstall", () => {
  test("marks all core services as running", async () => {
    const state = makeTestState();
    trackDir(state.homeDir);
    const origHome = setupLifecycleTest(state);
    ...
    try {
      ...
      teardownLifecycleTest(origHome);
    }
  });
});
