...
function seedRequiredAssets(homeDir: string): void {
	...
	writeFileSync(
		join(homeDir, 'data', 'assistant', 'opencode.jsonc'),
	...
	writeFileSync(
		join(homeDir, 'data', 'registry', 'automations', 'cleanup-logs.yml'),
		'schedule: "0 4 * * 0"\ndescription: cleanup logs\ncommand: ["echo","clean"]\n'
	);
	writeFileSync(
		join(homeDir, 'data', 'registry', 'automations', 'cleanup-data.yml'),
		'schedule: "0 5 * * 0"\ndescription: cleanup data\ncommand: ["echo","clean"]\n'
	);
	writeFileSync(
		join(homeDir, 'data', 'registry', 'automations', 'validate-config.yml'),
...
describe('validateSetupSpec', () => {
	...
	it('rejects invalid connection ID pattern', () => {
		const spec = makeValidSpec({
			connections: [{ id: '-invalid', name: 'Bad', provider: 'openai', baseUrl: '', apiKey: '' }]
		});
		const result = validateSetupSpec(spec);
		expect(result.valid).toBe(false);
...
describe('persistAkmConfig', () => {
	...
	it('writes the canonical typed shape for llm + embedding', () => {
		...
		expect(cfg).toEqual({
			...
			defaults: { engine: 'default', llmEngine: 'default' },
			embedding: {
	...
	it('preserves existing user keys and nested engine/defaults fields', () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				...
				bundles: { custom: { path: '/custom', writable: false } },
		...
		expect(cfg.bundles.custom).toEqual({ path: '/custom', writable: false });
		...
		// Existing defaults key survives; native engine selections are added.
		expect(cfg.defaults.embedding).toBe('myembed');
		expect(cfg.defaults.engine).toBe('default');
	...
	it('drops retired 0.8 keys on write', () => {
		mkdirSync(join(configDir, 'akm'), { recursive: true });
...
describe('performSetup', () => {
	...
	beforeEach(() => {
		...
		writeFileSync(
			...
			['OP_SETUP_COMPLETE=false', 'OPENAI_BASE_URL=', 'OP_OWNER_NAME=', 'OP_OWNER_EMAIL=', ''].join(
				'\n'
			)
		);
	...
	it('returns an error for invalid input', async () => {
		const result = await performSetup({ security: { uiLoginPassword: 'short' } } as SetupSpec);
	...
	it('writes akm config.json with llm and embedding', async () => {
		...
		// Canonical persisted AKM 0.9 shape.
		...
		expect(config.defaults.engine).toBe('default');
		...
		expect(config.bundles.stash).toEqual({ path: '/stash', writable: true });
		expect(config.defaultBundle).toBe('stash');
	});

	it('does not write the legacy migration-triggering akm config shape (I-3)', async () => {
		// Retired 0.8 fields are rejected by AKM 0.9 and must never survive persistence.
		const result = await performSetup(makeValidSpec());
		expect(result.ok).toBe(true);
		const config = JSON.parse(
			readFileSync(join(homeDir, 'config', 'akm', 'config.json'), 'utf-8')
		) as Record<string, unknown>;
		for (const key of ['profiles', 'llm', 'agent', 'stashes', 'stashDir', 'sources']) {
	...
	it('auto-enables host akm sharing when host AKM is available (no overlay, no personal-side write)', async () => {
		...
		try {
			...
			const hostCfgRaw = JSON.stringify({
				configVersion: '0.9.0',
				bundles: { stash: { path: join(fakeHome, 'akm'), writable: true } },
				defaultBundle: 'stash'
			});
			...
			// Assistant-side secondary bundle is present without becoming the default.
			const opCfg = JSON.parse(
				readFileSync(join(homeDir, 'config', 'akm', 'config.json'), 'utf-8')
			);
			expect(opCfg.bundles['host-akm']).toEqual({
				path: '/host-stash',
				writable: true,
				enabled: true
			});
			expect(opCfg.defaultBundle).toBe('stash');
	...
	it('enables sharing even when host has no akm config (profile import silently skipped)', async () => {
		...
		try {
			...
			const result = await performSetup(makeValidSpec({ hostAkm: true }));
			expect(result.ok).toBe(true);
			// Secondary bundle always present (written unconditionally).
			const opCfg = JSON.parse(
				readFileSync(join(homeDir, 'config', 'akm', 'config.json'), 'utf-8')
			);
			expect(opCfg.bundles['host-akm'].path).toBe('/host-stash');
			expect(opCfg.defaultBundle).toBe('stash');
	...
	it('completes setup with multiple connections', async () => {
		...
		const authJson = JSON.parse(
			readFileSync(join(homeDir, 'knowledge', 'secrets', 'auth.json'), 'utf-8')
	...
	it('disables an existing addon when the rerun spec sets it false', async () => {
		...
		const enable = await performSetup(
			makeValidSpec({
				addons: { discord: true },
	...
	it('preserves an existing assistant key across a rerun — rotating it would break every client', async () => {
		expect((await performSetup(makeValidSpec({ access: { assistantDirect: true } }))).ok).toBe(
			true
		);
	...
	it('writes every managed bind explicitly, so switching combinations always converges', async () => {
		expect(
			(
				await performSetup(
					makeValidSpec({ access: { networkAccess: true, guardianNetwork: true } })
				)
			).ok
		).toBe(true);
