...
// ── Tests: buildChannelCredentialEnvVarsFromMap ──────────────────────────

describe("buildChannelCredentialEnvVarsFromMap", () => {
  it("maps discord credentials from flat map", () => {
    const envVars = buildChannelCredentialEnvVarsFromMap({
      discord: {
        botToken: "bot-token-123",
        applicationId: "app-id-456",
      },
  ...
  it("maps slack credentials from flat map", () => {
    const envVars = buildChannelCredentialEnvVarsFromMap({
      slack: {
    ...
    expect(envVars.SLACK_BOT_TOKEN).toBe("xoxb-test");
    expect(envVars.SLACK_APP_TOKEN).toBe("xapp-test");
  });
});
