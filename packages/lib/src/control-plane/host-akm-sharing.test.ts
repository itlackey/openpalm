...
describe("enableHostAkmSharing", () => {
  ...
  it("imports host LLM engines when host config exists", () => {
    setHostAkmConfig({
      engines: { default: { kind: "llm", endpoint: "http://h/v1/chat/completions", model: "qwen" } },
    });
    writeFileSync(opConfig, "{}");
    const { profilesImported } = enableHostAkmSharing(state);
    expect(profilesImported).toContain("engines");
    const engines = readJson(opConfig).engines as Record<string, Record<string, unknown>>;
...
describe("disableHostAkmSharing", () => {
  ...
  it("does NOT touch the akm config bundle map", () => {
    ...
    disableHostAkmSharing(state);
    // Bundle entry untouched; enable/disable only changes the Compose mount.
    const cfg = readJson(opConfig);
    const bundles = cfg.bundles as Record<string, Record<string, unknown>>;
    expect(bundles["host-akm"].path).toBe("/host-stash");
  });
