 ...
 * type (ref `env/user`, akm >= 0.9.0 grammar): a whole `.env` file that akm
 * loads wholesale via `akm env run user -- <cmd>` / `akm env path user`. The
 * assistant entrypoint deliberately does not source it into the OpenCode
 * server environment.
 *
 ...
 *   data/akm/      — akm operational cache, data, and scheduler state
 */
...
/** akm ref for the user-managed environment file (0.9.0 `[bundle//]conceptId` grammar). */
...
export function buildAkmEnv(state: ControlPlaneState): NodeJS.ProcessEnv {
  return {
    ...
    AKM_STATE_DIR: `${state.dataDir}/akm/data/state`,
  };
}

/**
 * The five XDG-base akm env vars that MUST be set together (akm 0.9.0 —
 * AKM_BUNDLE_DIR replaced AKM_STASH_DIR with no fallback to the old name;
 * AKM_STATE_DIR carries task-scheduler invocation state).
 */
export const AKM_ENV_KEYS = [
  "AKM_BUNDLE_DIR",
  "AKM_CONFIG_DIR",
  "AKM_CACHE_DIR",
  "AKM_DATA_DIR",
