...
describe('resolveArtifactName', () => {
  ...
  it('errors clearly for an unsupported platform (win32/arm64 — no published artifact)', () => {
    expect(() => resolveArtifactName('win32', 'arm64')).toThrow(/Unsupported platform: win32\/arm64/);
  });

  it('errors clearly for any other unpublished platform/arch', () => {
    expect(() => resolveArtifactName('freebsd', 'x64')).toThrow(/Unsupported platform: freebsd\/x64/);
...
describe('resolveCacheRoot', () => {
  it('honors an explicit OPENPALM_CACHE_DIR override', () => {
    expect(resolveCacheRoot({ OPENPALM_CACHE_DIR: '/tmp/custom-cache' }, 'linux', '/home/user')).toBe(
      '/tmp/custom-cache',
    );
  });
  ...
  it('uses Library/Caches on darwin', () => {
    expect(resolveCacheRoot({}, 'darwin', '/Users/user')).toBe('/Users/user/Library/Caches/openpalm');
  });

  it('uses LOCALAPPDATA on win32 when set', () => {
    expect(
      resolveCacheRoot({ LOCALAPPDATA: 'C:\\Users\\user\\AppData\\Local' }, 'win32', 'C:\\Users\\user'),
...
describe('ensureCachedBinary', () => {
  ...
  it('fails closed on a checksum mismatch and does not cache the binary', async () => {
    ...
    await expect(ensureCachedBinary({ version, artifact, cacheRoot, fetchImpl, fs })).rejects.toThrow(
      /Checksum mismatch/,
    );
  ...
  it('fails closed when the binary download itself 404s', async () => {
    ...
    await expect(ensureCachedBinary({ version, artifact, cacheRoot, fetchImpl, fs })).rejects.toThrow(
      /Failed to download openpalm-cli-linux-x64 \(404\)/,
    );
  });
});
...
describe('runBinary', () => {
  it('propagates the child process exit code', () => {
    const spawn = () => ({ status: 3, error: undefined, signal: null }) as ReturnType<typeof import('node:child_process').spawnSync>;
    expect(runBinary('/fake/bin', [], spawn)).toBe(3);
  });

  it('returns 0 for a clean exit', () => {
    const spawn = () => ({ status: 0, error: undefined, signal: null }) as ReturnType<typeof import('node:child_process').spawnSync>;
    expect(runBinary('/fake/bin', [], spawn)).toBe(0);
  });

  it('rethrows a spawn error (e.g. ENOENT)', () => {
    const spawn = () => ({ status: null, error: new Error('spawn ENOENT'), signal: null }) as ReturnType<
  ...
  it('returns a non-zero code when the child died from a signal', () => {
    const spawn = () => ({ status: null, error: undefined, signal: 'SIGKILL' }) as ReturnType<
      typeof import('node:child_process').spawnSync
    >;
