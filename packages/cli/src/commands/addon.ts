...
function requireKnownAddon(name: string): void {
	...
	const hint =
		available.length > 0 ? ` Run \`openpalm addon list\` to see the available addons.` : '';
	if (!available.includes(name)) {
		throw new Error(
			`Addon "${name}" is not available. Known addons: ${available.join(', ') || '(none)'}.${hint}`
		);
	}
}
...
export async function runAddonEnableAction(name: string): Promise<void> {
	...
	try {
		const mutation = setAddonEnabled(state.homeDir, name, true, state);
		...
		try {
			...
			await runComposeWithPreflight(nextState, ['up', '-d', ...mutation.services], lock);
...
export async function runAddonDisableAction(name: string): Promise<void> {
	...
	try {
		...
		const mutation = setAddonEnabled(state.homeDir, name, false, state);
		if (!mutation.ok) throw new Error(mutation.error);
...
const enableCmd = defineCommand({
	...
	async run({ args }) {
		...
		if (!name)
			throw new Error(
				'Addon name is required. Run `openpalm addon list` to see the available addons.'
			);
