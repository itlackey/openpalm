// Repo hygiene guard: the electron build-artifact dir `packages/electron/dist/`
// is regenerated by `bun run --cwd packages/electron bundle` — the release
// workflow rebuilds it before packaging/publish. It must never be git-tracked:
// a tracked stale bundle is silent review noise (e.g. a contributor grepping
// the repo for a removed symbol gets a false hit from a stale compiled file),
// and gitignore already declares it ignored, so tracking it is pure drift.
//
...
describe('electron build-artifact dirs are not git-tracked', () => {
  const BUILD_DIRS = ['packages/electron/dist/'];
