...
View and manage external API connections (secrets stored in `config/stack/stack.env`):
...
- **set** (`POST /admin/connections`) = patch `config/stack/stack.env` with new API key values. Accepts a map of key/value pairs. Use this when the user needs to add or rotate an API key.
