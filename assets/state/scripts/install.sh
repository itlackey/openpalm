...
mkdir -p "$OPENPALM_DATA_HOME"/{postgres,qdrant,openmemory,opencode}
mkdir -p "$OPENPALM_CONFIG_HOME"
mkdir -p "$OPENPALM_STATE_HOME"/{gateway,rendered,logs,tmp,observability,backups}
mkdir -p "$OPENPALM_STATE_HOME"/rendered/{caddy,snippets,env}
...
cat > "$OPENPALM_STATE_HOME/rendered/env/gateway.env" <<'EOF'
# generated by admin
EOF
cat > "$OPENPALM_STATE_HOME/rendered/env/openmemory.env" <<'EOF'
# generated by admin
EOF
cat > "$OPENPALM_STATE_HOME/rendered/env/postgres.env" <<'EOF'
# generated by admin
EOF
cat > "$OPENPALM_STATE_HOME/rendered/env/qdrant.env" <<'EOF'
# generated by admin
EOF
cat > "$OPENPALM_STATE_HOME/rendered/env/opencode.env" <<'EOF'
# generated by admin
EOF
cat > "$OPENPALM_STATE_HOME/rendered/env/channels.env" <<'EOF'
# generated by admin
EOF
