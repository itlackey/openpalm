...
if [ "$enabled" = "1" ]; then
  ...
  # Loopback-only and deliberately unauthenticated: the opencode binary keys
  # Basic auth on a raw OPENCODE_SERVER_PASSWORD env, which this container
  # never receives (it gets only the _FILE variable for its upstream calls).
