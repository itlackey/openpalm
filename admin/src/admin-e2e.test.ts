...
beforeAll(async () => {
  ...
  // Copy config files
  copyFileSync(join(REPO_ROOT, "assets/config/opencode/opencode.jsonc"), join(configDir, "opencode.jsonc"));
  copyFileSync(join(REPO_ROOT, "assets/shared/caddy/Caddyfile"), join(caddyDir, "Caddyfile"));
