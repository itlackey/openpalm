...
| 1.1 | DONE | merged into automation/fable-remediation, independently verified. Ran both commands in /home/founder3/code/github/itlackey/openpalm/.claude/worktrees/fable/_base. `git merge-base --is-ancestor automation/1.1 automation/fable-remediation` exited 0 (EXIT=0). `git log automation/fable-remediation --oneline -3` top line is `7183d775 fix(1.1): delete self-defeating CI gate + flake-retry loop` (a 1.1 commit). Corroborated: automation/1.1 tip = 8c28d066, which equals `git merge-base automation/1.1 automation/fable-remediation`, and `git branch automation/fable-remediation --contains automation/1.1` lists automation/fable-remediation. Both required conditions hold. |
...
| 1.3 | DONE | Completed and merged into automation/fable-remediation, independently verified. Ran both commands in /home/founder3/code/github/itlackey/openpalm/.claude/worktrees/fable/_base. `git merge-base --is-ancestor automation/1.3-v2 automation/fable-remediation` exited 0 (verified via EXIT_CODE=0), confirming automation/1.3-v2 is an ancestor of automation/fable-remediation. `git log automation/fable-remediation --oneline -3` output: e7545e32 fix(1.3): truthful core-principles.md + AGENTS.md/CLAUDE.md refresh; 46621456 docs(1.3): truthful five-tree layout + purge phantom-mechanism prose; 58dee519 docs: status 3.3 -> DONE. The top two commits are explicit 1.3-scoped commits. Both claims verified. |
...
| 2.2 | DONE | Completed and merged into automation/fable-remediation, independently verified. Verified in worktree /home/founder3/code/github/itlackey/openpalm/.claude/worktrees/fable/_base. `git merge-base --is-ancestor automation/2.2-v2 automation/fable-remediation` exited 0 (2.2-v2 tip = 29388bc5, confirmed contained in fable-remediation via `git branch --contains`). `git log automation/fable-remediation --oneline -3` top commit is `fb71699c fix(2.2): finish compose-driver collapse (one driver, net-negative)` — a 2.2 commit. Both facts hold. |
| 2.3 | DONE | TDD green, Opus-approved, merged into automation/fable-remediation, independently verified in 1 round(s). `git merge-base --is-ancestor automation/2.3 automation/fable-remediation` exited 0 (ancestor confirmed). automation/2.3 tip is 3de262f5, which appears in the fable-remediation log. `git log automation/fable-remediation --oneline -3` shows two 2.3 commits in its top 3: `c48e01df fix(2.3): route compose normalization through compose config --format` and `3de262f5 refactor(lib): route bind-mount discovery through compose config --format json (plan 2.3)`. |
| 3.1 | DONE | TDD green, Opus-approved, merged into automation/fable-remediation, independently verified in 2 round(s). Ran both commands in /home/founder3/code/github/itlackey/openpalm/.claude/worktrees/fable/_base. `git merge-base --is-ancestor automation/3.1 automation/fable-remediation` exited 0 (automation/3.1 is an ancestor of automation/fable-remediation). `git log automation/fable-remediation --oneline -3` returned: 7978cd46 fix(3.1): Install-lock correctness and coverage / 4dfa03ee test(3.1): re-export isProcessAlive in deploy.test harness stub / 8af9b71d fix(install-lock): never steal a live-holder lock; hold it across applyStack (3.1). The HEAD commit is a 3.1 commit, confirming 3.1 work is present and merged into fable-remediation. |
...
| 3.3 | DONE | TDD green, Opus-approved, merged into automation/fable-remediation, independently verified in 1 round(s). Verified both claims in worktree /home/founder3/code/github/itlackey/openpalm/.claude/worktrees/fable/_base. 1. `git merge-base --is-ancestor automation/3.3-optionB automation/fable-remediation` exited 0 (confirmed via `echo "EXIT=$?"` printing EXIT=0). So automation/3.3-optionB IS an ancestor of automation/fable-remediation. 2. `git log automation/fable-remediation --oneline -3` output: d9826d87 fix(3.3): Electron update-check requires a matching installer asset (3 / a5aa837c docs: status 2.3 -> DONE / c48e01df fix(2.3): route compose normalization through compose config --format. The top commit d9826d87 is the 3.3 commit ("fix(3.3): Electron update-check requires a matching installer asset"), confirming a 3.3 commit is present in the recent log. The second-newest 2.3 commits are also present, consistent with the fable-2324 work. |
