...
│   ├── stack.env            # System-managed infrastructure config (paths, UID/GID, image, network)
│   ├── secrets.env          # Staged user secrets merged with system-managed DB/channel secrets
...
├── secrets/                 # Persisted system-managed secrets (not user-edited)
│   ├── system-secrets.env   # POSTGRES_PASSWORD (persisted across restarts)
│   └── channel-secrets.env  # CHANNEL_<NAME>_SECRET values (persisted across restarts)
└── audit/
...
Generated by the admin on every apply. Never user-edited. Contains:

- **XDG paths:** `OPENPALM_CONFIG_HOME`, `OPENPALM_DATA_HOME`, `OPENPALM_STATE_HOME`, `OPENPALM_WORK_DIR`
- **User/Group:** `OPENPALM_UID`, `OPENPALM_GID`, `OPENPALM_DOCKER_GID` (auto-detected from process and `/var/run/docker.sock`)
...
- **Database:** `POSTGRES_PASSWORD` (auto-generated, persisted in `STATE_HOME/secrets/system-secrets.env`)
- **Channel HMAC keys:** `CHANNEL_<NAME>_SECRET` (auto-generated per channel, persisted in `STATE_HOME/secrets/channel-secrets.env`)

For fresh installs (before the admin has run), `scripts/dev-setup.sh --seed-env` seeds this file with auto-detected values.
...
auto-generates a channel HMAC secret, persists it under
`STATE_HOME/secrets/channel-secrets.env`, and writes it into `stack.env` on the
next apply. The guardian reads channel secrets from the bind-mounted `stack.env`.
...
`CHANNEL_<NAME>_SECRET` values are generated by admin logic, persisted in
`STATE_HOME/secrets/channel-secrets.env`, and written into `stack.env` on every apply.
