...
2. verify at least one provider is configured in OpenCode auth state or `~/.openpalm/knowledge/vaults/secrets/`
3. confirm the provider endpoint is reachable from Docker if you use a local model server

Useful checks:

```bash
ls ~/.openpalm/knowledge/vaults/secrets
...
- check `~/.openpalm/knowledge/vaults/secrets/` for the relevant channel HMAC secret file and verify the service has a matching `*_FILE` grant
...
live non-secret values come straight from `config/stack/stack.env`; service secrets come from `knowledge/vaults/secrets/`.
...
the compose files under `~/.openpalm/config/stack/`, non-secret `stack.env`, and file-based secrets under `knowledge/vaults/secrets/`.
