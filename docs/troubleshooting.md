...
4. Verify the Caddy config has the admin route: check `~/.local/state/openpalm/caddy.json`
...
2. Check assistant logs: `docker compose -f ~/.local/state/openpalm/docker-compose.yml logs assistant --tail=50`
3. Look for API rate limit or authentication errors in the logs.
4. Verify the assistant container is healthy: `docker compose -f ~/.local/state/openpalm/docker-compose.yml ps assistant`
