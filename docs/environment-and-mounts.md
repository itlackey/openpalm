...
- **`stack.env`** — ALL system-managed config. Auto-generated by admin on every apply; never user-edited. Contains:
  - Infrastructure: paths, UID/GID, image namespace/tag, networking, OpenMemory URLs
  - Database: `POSTGRES_PASSWORD` (generated by admin, persisted in `STATE_HOME/secrets/`)
  - Channel HMAC keys: `CHANNEL_<NAME>_SECRET` (generated per channel, persisted in `STATE_HOME/secrets/`)
...
Database credentials and channel HMAC secrets are generated by the admin, persisted under `STATE_HOME/secrets/`, and written into `stack.env` on every apply. Users typically only need to edit `CONFIG_HOME/secrets.env` for tokens and LLM keys.
...
**System-managed** (persisted in `STATE_HOME/secrets/`, written into `STATE_HOME/artifacts/stack.env`):

| Secret | Consumed By | Purpose |
|---|---|---|
| `POSTGRES_PASSWORD` | postgres | Database password — generated by admin, persisted in `system-secrets.env` |
...
They are written into `STATE_HOME/artifacts/stack.env` on each apply.
...
| `OPENPALM_DOCKER_GID` | docker socket GID | `statSync('/var/run/docker.sock').gid` |
...
- **Docker socket** — Only the admin service mounts `/var/run/docker.sock`. The
  assistant is explicitly denied Docker access.
...
- **Secrets isolation** — Most containers receive only the secrets they
  ...
  by the admin and stored in `STATE_HOME/secrets/channel-secrets.env`; they
