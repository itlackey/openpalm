...
- **Guardian** (`entrypoint.sh:46-95`) and **portal** (`start.sh:22-32`) re-install their (exact-pinned) packages at boot; guardian's is a no-op given its skip-if-current check, portal's is a hard-fail on cold start. Their manifests are already exact-pinned — the design has been trending toward "pin at release, not runtime" everywhere except the assistant's five ranged tools.
...
5. **Remove the two-manifest drift**: stop seeding an operator `tools/package.json` that differs from the image. Cleanest is to **drop the `data/*/tools` bind mounts** so operators run exactly what the image ships; if a per-tool override is retained as an *advanced, documented opt-in*, it must seed the exact image version (never a range).
6. **Host-side UI/skeleton hot-swap is a separate decision.** `npm-bundle-updater` (via `ui-server.ts:224-237`, `main.ts:329-342`) updates `OP_HOME/data/ui` and `OP_HOME/system` from npm on the **host** (CLI/Electron), independent of the container installs above, and exists to ship UI/config fixes without a full image pull. If the intent is truly "image is the only update path," gate or remove it too so the host UI tracks the installed version; otherwise keep it deliberately and document it as a distinct, intentional channel. (It has fail-closed sha512 verification, so it is not a security concern — only a reproducibility decision.)

**Trade-off (state plainly).** Removing runtime installs **costs** the operator-editable `tools/package.json` override and the assistant's live tool advancement (auto-picking up patch/minor bumps without a new image). It **gains** reproducibility (what's in the image is what runs), removes the boot-time npm/bun network dependency, deletes ~120 lines of soft/hard-failure install logic plus the cache volumes and guardian `.npmrc` plumbing, and closes the drift-bug class shown above by construction. This matches the direction guardian and portal already took and is the recommended path.
...
1. **G1** (critical) — relocate delegated secrets out of the akm stash, stop sourcing env:user into the OpenCode server process, and deliver the guardian's `auth.json` via a compose secret. The env-sourcing change ships in a new assistant image, so **sequence it with E2**.
2. **E2** (pairs with G1) — remove the assistant/guardian runtime installs, bake everything into the image (incl. the missing skeleton bake and pinned plugin), exact-pin the assistant tools. Same new image carries G1's entrypoint change.
3. **C1, D1, C3, B1** — high-impact, small, low-risk (purge; day-2 Docker errors; password = live file source + `reset-password`, no restart; Windows checksum).
4. **A1** — repair or deliberately deprecate the npm channel.
5. **G3 + G2** — portals default-deny with a `"*"` explicit opt-in, and repair the assistant denylist so its "ask"/"deny" patterns actually match; ship with a release note. (G4 is by-design — no work.)
6. **E4** — free cosign keyless signing with an in-CI verify step so it can't silently regress.
7. **B2, B3, B5, G6, G7** — small Windows/moderation/edge hardening.
8. **E1, E3, F1, C2** — versioning pin; minimal voice publish tie-in; rootless install-path chown; CLI `doctor`.
9. **F2, B4, G5/G6 (guardian plugin)** — larger cross-runtime / arm64 / a future dedicated guardian OpenCode plugin for spend visibility and tool-output screening, with the noted unknowns (Bun arm64 target; Podman compose provider; token visibility).
