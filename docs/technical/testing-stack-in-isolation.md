...
mkdir -m 700 -p .dev-test/stash/vaults/secrets
...
printf '%s\n' 'dev-admin-token' > .dev-test/stash/vaults/secrets/op_ui_login_password
chmod 600 .dev-test/stash/vaults/secrets/op_ui_login_password
```
...
# Login stores an opaque op_session cookie; the cookie value is not the password.
curl -c /tmp/openpalm-test-cookie -X POST http://localhost:9100/admin/auth/login \
  -H 'content-type: application/json' \
  -d '{"password":"dev-admin-token"}'

# Should return { ok: true, opencode: true } — assistant is running.
curl -b /tmp/openpalm-test-cookie http://localhost:9100/admin/health
