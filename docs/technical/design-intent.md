...
- Runtime behavior is composed from:
  ...
  - non-secret environment file (`config/stack/stack.env`) and file-based service secrets (`config/stack/secrets/`),
...
- `stash/vaults/` is the user-managed secrets boundary (`user.env`). System secrets live as file-based grants under `config/stack/secrets/`; `stack.env` is non-secret.
