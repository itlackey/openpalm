...
Platform packages (root, `packages/lib`, `packages/admin`, `core/guardian`, `packages/cli`, `packages/channels-sdk`) share a coordinated version bumped by `scripts/bump-platform.sh` and published from the release workflow. Channel packages (`packages/channel-*`) are versioned independently via per-package publish workflows. Cross-references between groups use real semver ranges and are updated manually when a dependency's API changes.
