...
| `OP_WORKSPACE_ORIGIN` | The PUBLIC address of that listener, when a browser cannot work it out. Empty by default and correct empty for every deployment where the workspace is another port of the host the browser typed (desktop, LAN, Tailscale) — `/advanced` composes that from the page it is on. Set it when something in front of the install publishes the workspace under a name of its own (Caddy on a second subdomain): a bare origin, scheme + host + optional port and nothing more; a value carrying a path, query, or credentials is ignored and the derived address stands. It must be same-site with the UI, or the browser will not carry the session into the frame. Outranks the port and any provider declaration; a remote provider that moves the workspace declares its own (`workspaceOrigin` in `remote-providers.ts`) |
