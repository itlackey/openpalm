...
| `OP_WORKSPACE_ORIGIN` | The PUBLIC address of that listener, when a browser cannot work it out. Empty by default, and correct empty for every deployment where the workspace is another port of the host the browser typed (desktop, LAN, Tailscale) — `/advanced` composes that from the page it is on. Set it when an edge publishes the workspace at an address the derived one cannot reach: a bare origin, scheme + host + optional port and nothing more; a value carrying a path, query, or credentials is ignored and the derived address stands. Keep the HOSTNAME the same as the UI's — the session cookie is what authenticates the workspace and cookies are host-scoped, so a different hostname gets no credential. Outranks the port and any provider declaration; a remote provider that moves the workspace declares its own (`workspaceOrigin` in `remote-providers.ts`) |
| `OP_SESSION_COOKIE_DOMAIN` | Parent domain for the UI session cookie. Empty by default (host-only), which is correct wherever the workspace is another PORT of the UI's hostname — cookies ignore ports, so desktop, LAN and Tailscale need nothing. Required for the reverse-proxy layout that puts the workspace on a second HOSTNAME (`app.example.com` + `code.example.com`), which Cloudflare Tunnel forces since it proxies a fixed port set the workspace port is not in. Setting it sends the session to every subdomain of that domain — a real widening, never inferred |
