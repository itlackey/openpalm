...
| `/advanced` | Assistant | auth | Embedded OpenCode web UI — framed at the connection when it is itself an OpenCode origin, otherwise at the server's `opencodeWorkspace` advertisement (the same OpenCode at its own published origin, since the locked `/oc` connection is an API proxy and OpenCode's UI is a root-mounted SPA). Falls back to a native chat surface where neither is framable. `routes/(app)/advanced/embeddable.ts` owns the whole classification and is the place to read it. Mounts `ChatNavbar` (chat chrome composition) |
