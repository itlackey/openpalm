...
| `/advanced` | Assistant | auth | Embedded OpenCode web UI when the active connection is a reachable, credential-free OpenCode ORIGIN (`embeddable.ts`); the locked `/oc` same-origin connection and credentialed ones get the native chat surface instead — OpenCode's UI is a root-mounted SPA and cannot be framed from a path prefix. Mounts `ChatNavbar` (chat chrome composition) |
