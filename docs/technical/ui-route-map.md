...
| `/advanced` | Assistant | auth | Embedded OpenCode web UI — framed at the connection when it is itself an OpenCode origin, otherwise at the **workspace listener** the server advertises as `opencodeWorkspace` (the locked `/oc` connection is an API proxy, and OpenCode's UI is a root-mounted SPA, so it needs an origin whose root is OpenCode). That advertisement is either an absolute origin someone authoritative declared (`OP_WORKSPACE_ORIGIN`, or the remote provider fronting the install) or a bare port the browser completes with its own host. The address is probed before it is framed; nothing answering shows a notice and a link to `/chat` — this route has **no** chat fallback, deliberately. A workspace on another hostname opens with a one-minute single-use ticket that the listener trades for a cookie of its own (`lib/server/workspace-ticket.ts`). `routes/(app)/advanced/embeddable.ts` owns the whole classification and is the place to read it. Mounts `ChatNavbar` (chat chrome composition) |
...
| `/api/workspace/ticket` | Assistant | requireAdmin (session only — a ticket IS the session) | POST-only: mints a one-minute, single-use credential that carries this browser's session to a workspace listener published on a different hostname, where cookies do not reach. Used only by `/advanced`, and only for that case |
