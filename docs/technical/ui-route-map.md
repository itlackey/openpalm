...
| `/advanced` | Assistant | auth | Embedded OpenCode web UI — framed at the connection when it is itself an OpenCode origin, otherwise at the **workspace listener** the server advertises as `opencodeWorkspace` (the locked `/oc` connection is an API proxy, and OpenCode's UI is a root-mounted SPA, so it needs an origin whose root is OpenCode). That advertisement is either an absolute origin someone authoritative declared (`OP_WORKSPACE_ORIGIN`, or the remote provider fronting the install) or a bare port the browser completes with its own host; either way it must share the UI's hostname, since the session cookie is what authenticates it. The address is probed before it is framed; nothing answering shows a notice and a link to `/chat` — this route has **no** chat fallback, deliberately. `routes/(app)/advanced/embeddable.ts` owns the whole classification and is the place to read it. Mounts `ChatNavbar` (chat chrome composition) |
...
| `/oc/[...path]` | Assistant | requireAdmin (session only — no capability) | Transparent same-origin pass-through to **this process's own** OpenCode (`getAssistantOpencodeTarget()`), OpenCode's Basic auth attached server-side, always. Not to be confused with Guardian's own `/oc/*` — a different server, port, and auth model, reached only by external/portal clients, never by this route; see [`api-spec.md`](api-spec.md#oc-proxy-disambiguation) for the full disambiguation |
