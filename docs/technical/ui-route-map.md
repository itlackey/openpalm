...
| `/advanced` | Assistant | auth | Embedded OpenCode web UI. The locked default connection frames the same-origin `/_opencode` workspace proxy; another connection that is itself a framable OpenCode origin frames itself; anything else falls back to the `opencodeWorkspace` direct-port advertisement, then to a native chat surface. `routes/(app)/advanced/embeddable.ts` (`resolveFrameBase`) owns the whole classification and is the place to read it. Mounts `ChatNavbar` (chat chrome composition) |
...
| `/_opencode/[...path]` | Assistant | requireAdmin (session only — no capability) | Same-origin pass-through to the same OpenCode as `/oc`, serving its **web UI** for `/advanced`'s iframe. The HTML document (and only the document) gets a client shim injected and the upstream CSP's `script-src` extended with that shim's hash; everything else streams through untouched. Served with `X-Frame-Options: SAMEORIGIN` and exempt from the landing/login document redirects (it answers 401 instead) — see `hooks.server.ts`. `$lib/server/opencode-workspace.ts` explains why OpenCode's SPA cannot simply be pointed at a sub-path |
| `/assets/[...path]` | Assistant | requireAdmin (session only — no capability) | GET-only 1:1 proxy for OpenCode's static bundle at this origin's **root**. Not a namespace grab: OpenCode's Vite build has `base: "/"` baked in and reaches its assets through `<script>`/`<link>` tags, `modulepreload` links, and CSS `url()` — none interceptable by the shim, and a missed preload breaks lazy routes outright. OpenPalm's own build output is `/_app/*` |
