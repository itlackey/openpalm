...
| `/advanced` | Assistant | auth | Embedded OpenCode web UI. The locked default connection frames the static bundle at `/opencode-ui/` (below); another connection that is itself a framable OpenCode origin frames itself; anything else renders a notice pointing at /chat. `routes/(app)/advanced/embeddable.ts` (`resolveFrameBase`) owns the whole classification and is the place to read it. Mounts `ChatNavbar` (chat chrome composition) |
...
| `/oc/[...path]` | Assistant | requireAdmin (session only — no capability) | Transparent same-origin pass-through to **this process's own** OpenCode (`getAssistantOpencodeTarget()`), Basic auth always attached server-side (OpenCode authenticates every caller). Not to be confused with Guardian's own `/oc/*` — a different server, port, and auth model, reached only by external/portal clients, never by this route; see [`api-spec.md`](api-spec.md#oc-proxy-disambiguation) for the full disambiguation |
| `/opencode-ui/*` | Assistant | public shell (data flows through `/oc`) | OpenCode's own web UI as **static files**, built from source pinned to the same version as the `opencode` runtime with a real base path (`scripts/opencode-web/build.sh`; bundle is gitignored, never committed). Real files are served by the static handler; `routes/opencode-ui/[...path]/+server.ts` is the standard SPA fallback that answers the app's client-side session routes with the shell. Framable same-origin and exempt from the landing/login document redirects (`isOpencodeWebPath` in `hooks.server.ts`). Every API call the app makes rides the session-gated `/oc` |
