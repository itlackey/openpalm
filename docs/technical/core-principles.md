...
**Rule:** CLI/admin writes backup snapshots here before destructive lifecycle changes. A lifecycle safety backup includes `config/`, `system/`, `state/`, `knowledge/`, and `workspace/`; it excludes service-owned `data/` and regenerable `cache/`. Because `data/<service>/` is out of scope, that service's own credentials are too — `state/env/<service>.env` is left out and named in the snapshot's `.backup-complete` marker, so the two are backed up and restored together (§5) rather than half-restored. `uninstall --purge` removes all seven `OP_HOME` trees, including `state/` and `cache/`. Ownership repair covers durable/user/state roots and excludes regenerable cache content. Backups remain user-accessible durable operator state.
