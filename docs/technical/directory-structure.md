...
| **vault/** | Admin | Secrets: `user.env` (LLM keys), `system.env` (admin token, HMAC, paths) |
...
│   └── system.env                     # System secrets: admin token, HMAC keys, paths
│
├── config/
...
### `system.env` -- system-managed config
...
auto-generates a channel HMAC secret and writes it into `vault/system.env`.
...
Component definitions are cataloged in the `registry/` directory and bundled into
the admin image at build time. Components are installed from the registry via the
admin API or manually by placing a directory with `compose.yml` in
`config/components/`. No container rebuild required. The admin's apply endpoint
scans the directory and runs compose operations against discovered files.
...
`vault/system.env` on every apply.
