...
> The `admin` container also runs automatic maintenance jobs (image pulls, health checks, database maintenance, log rotation). See [Admin README](../../admin/README.md#system-maintenance-cron-jobs) for details.
...
The `admin` container includes a system cron job that periodically pulls image updates and restarts services. Check `~/.local/state/openpalm/observability/maintenance/` for update logs. See [Admin README](../../admin/README.md#system-maintenance-cron-jobs) for the full cron schedule.
