...
`OP_UI_LOGIN_PASSWORD`, `OPENCODE_SERVER_PASSWORD`, bind addresses, image
tags) are generated by setup/admin tooling and written into
`config/stack/stack.env` — you do not normally edit them manually.
...
1. Update `OP_UI_LOGIN_PASSWORD` in `~/.openpalm/config/stack/stack.env`
