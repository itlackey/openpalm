...
`data/` directly, and stack runtime files now live under `~/.openpalm/stack/`.
...
System-managed values (`CHANNEL_*_SECRET`, `OP_*` infrastructure vars, `OP_ADMIN_TOKEN`)
are generated by the admin and written into `vault/stack/stack.env` -- you don't need
to set them.

**After editing** -- rerun the same compose command or restart the services that
consume the changed values.
