...
- **`~/.openpalm/knowledge/vaults/secrets/`** -- System-managed service secret files granted through Compose `secrets:` and exposed as `*_FILE` variables.
...
`knowledge/vaults/secrets/` with narrow service grants. `stack.env` must not contain
...
- Put secret values in `~/.openpalm/knowledge/vaults/secrets/` and non-secret settings in `~/.openpalm/config/stack/stack.env`
...
1. Update OpenCode auth state through the admin provider flow or write the provider secret file under `~/.openpalm/knowledge/vaults/secrets/`
2. Restart the services that use it, such as `assistant`: `docker compose restart assistant`

**Add a new LLM provider:**
1. Add the API key through the admin provider flow or a file-based provider secret under `~/.openpalm/knowledge/vaults/secrets/`
...
1. Update `~/.openpalm/knowledge/vaults/secrets/op_ui_login_password`
