...
- #301 also depends on concurrent secrets/auth changes from roadmap issue #300 where component config touches `@sensitive` fields, token splits, and the new `vault/user.env` plus `vault/system.env` model. The component plan should define clean handoff points rather than duplicating that implementation.
- Keep the ownership split from `docs/technical/core-principles.md` explicit: `config/openpalm.yml` remains the user-editable stack config, `config/components/core.yml` and `config/components/admin.yml` remain the system-managed core overlays, and any `data/components/` instance copies or `data/components/enabled.json` state are service-managed runtime artifacts rather than a second user-facing config authority.
...
3. Convert core assets and setup scripts to the new filesystem contract.
   - Rewrite `assets/docker-compose.yml`, `assets/admin.yml`, `assets/Caddyfile`, `assets/secrets.env`, `assets/secrets.env.schema`, and `assets/stack.env.schema` into the new `config/components/` plus `vault/` mental model.
   - Update `scripts/dev-setup.sh` to seed `~/.openpalm/` layout, both vault env files, and cache directories.
   - Ensure the assistant mounts only `vault/user.env` read-only and no non-admin service mounts the full vault directory.
...
3. Build instance persistence and compose assembly in lib.
    ...
    - Create helpers that assemble compose args in the new order: vault env files first, core/admin overlays next, then enabled component instance overlays with each instance `.env`.
...
2. Remove or deprecate channel-specific admin endpoints.
   - Replace `/admin/channels/*` and registry install/uninstall flows that write raw `CONFIG_HOME/channels/*.yml` files.
...
2. Implement `openpalm migrate` for the combined 0.9.x to 0.10.0 transition.
   ...
   - Move directories into `~/.openpalm/`, split env files into `vault/user.env` and `vault/system.env`, convert legacy channel overlays into component-era locations, validate, deploy, and preserve old directories until optional cleanup.
...
- Detect existing `CONFIG_HOME/channels/*.yml` installations and block silent partial upgrades.
...
- `assets/docker-compose.yml:1`, `assets/admin.yml:1`, and `assets/Caddyfile:23` - core asset files carrying old paths, old ports, and current channel-routing assumptions.
