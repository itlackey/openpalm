...
1. **E3** — wizard UX 3-judge gate re-pass (needs live dev server + chrome-devtools; not headless-automatable).
2. **GitHub issue hygiene** — body rewrites + move #398→0.13.0 + close fixed bugs #481–#485.
3. **D5a mDNS image-baking** — the mDNS sidecars are default-OFF and install avahi via `apk add` at container boot with `network_mode: host` (the boot-install anti-pattern we removed for portals in D0b, plus it breaks isolation). Needs avahi baked into an image before it is production-shippable. Partitioning disposition itself is solid.
