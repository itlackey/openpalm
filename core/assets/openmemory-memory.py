...
Patched by OpenPalm to read vector_store config from /app/default_config.json,
enabling embedded (file-based) Qdrant via the `path` config key.

Original: /usr/src/openmemory/app/utils/memory.py
"""
...
def _load_config_file():
    """Load default_config.json from the mounted config file."""
...
def get_default_memory_config():
    ...
    Reads vector_store config from /app/default_config.json (mounted by OpenPalm)
    to support embedded Qdrant via path config. Falls back to hardcoded defaults
    ...
    # Override vector_store from config file if present
    file_config = _load_config_file()
    if file_config and "mem0" in file_config:
        mem0 = file_config["mem0"]
        if "vector_store" in mem0:
            config["vector_store"] = mem0["vector_store"]
            print(f"Loaded vector_store config from default_config.json: {json.dumps(config['vector_store'])}")
        if "llm" in mem0:
            config["llm"] = mem0["llm"]
        if "embedder" in mem0:
            config["embedder"] = mem0["embedder"]
...
def get_memory_client(custom_instructions: str = None):
    ...
    try:
        # Start with default configuration (includes vector_store from config file)
        ...
        try:
            ...
            if db_config:
                ...
                if "mem0" in json_config:
                    ...
                    # Update vector_store configuration if available in database
        ...
        if _memory_client is None or _config_hash != current_config_hash:
            ...
            print(f"Vector store config: {json.dumps(config.get('vector_store', {}))}")
            try:
