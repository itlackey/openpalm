"""
OpenMemory memories router — patched by OpenPalm.

Auto-creates users on first REST API access so the API works without
prior MCP provisioning. Uses `get_or_create_user` from app.utils.db,
the same helper the MCP server uses.

Original: /usr/src/openmemory/app/routers/memories.py
"""
import logging
