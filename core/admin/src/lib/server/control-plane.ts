...
/**
 * Persist system-managed secrets to the given secrets directory.
 */
function persistSystemSecretsTo(state: ControlPlaneState, targetSecretsDir: string): void {
  mkdirSync(targetSecretsDir, { recursive: true });
  const lines = [
    "# OpenPalm System Secrets — system-managed, do not edit",
    "",
    `POSTGRES_PASSWORD=${state.postgresPassword}`
  ];
  writeFileSync(`${targetSecretsDir}/system-secrets.env`, lines.join("\n") + "\n");
}

/**
 * Persist all current channel secrets to the given secrets directory.
 ...
 */
function persistChannelSecretsTo(state: ControlPlaneState, targetSecretsDir: string): void {
  mkdirSync(targetSecretsDir, { recursive: true });
  const lines = [
  ...
  writeFileSync(`${targetSecretsDir}/channel-secrets.env`, lines.join("\n") + "\n");
}
  ...
  for (const entry of entries) {
    ...
    if (existsSync(intentPath)) {
      try {
        ...
        // intent unreadable — fall through to cleanup
      }
    }
    // Always just clear the stale backup — startup re-apply will regenerate
    // STATE_HOME from whatever CONFIG_HOME currently contains.
 ...
 *   STATE_HOME/artifacts/  — generated compose, manifest, env files (not user-edited)
 *   STATE_HOME/secrets/    — system-managed secrets (not user-edited)
...
export function persistArtifacts(state: ControlPlaneState): void {
  ...
  const pendingSecrets = `${state.stateDir}/secrets.pending`;
  ...
  try {
    ...
    mkdirSync(pendingSecrets, { recursive: true });
    ...
    // Secrets → pending (inside the atomic swap, not directly to live)
    persistSystemSecretsTo(state, pendingSecrets);
    ...
    persistChannelSecretsTo(state, pendingSecrets);
    ...
    atomicSwap(`${state.stateDir}/secrets`, true);
    ...
    rmSync(pendingSecrets, { recursive: true, force: true });
