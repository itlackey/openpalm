/**
 * #491 — standalone CLI entrypoint contract for the Discord portal package.
...
describe('discord portal bin contract (#491)', () => {
  test('package.json declares the bin entrypoint, ships bin/ in files, and stays Bun-only', () => {
    ...
    expect(pkg.bin?.['openpalm-discord-portal']).toBe('bin/openpalm-discord-portal.ts');
  ...
  test('bin wrapper has a bun shebang and boots the portal class', () => {
    const wrapper = readRelative('bin/openpalm-discord-portal.ts');
  ...
  test('bin exits non-zero with startup_error when no principal secret is configured', async () => {
    const binPath = `${ROOT_DIR}/bin/openpalm-discord-portal.ts`;
